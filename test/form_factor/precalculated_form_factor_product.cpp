...
TEST_CASE("PrecalculatedFormFactorProduct::evaluate") {
    for (unsigned int ff1 = 0; ff1 < get_count(); ++ff1) {
        for (unsigned int ff2 = 0; ff2 < get_count(); ++ff2) {
            const FormFactor& ff1_obj = storage::get_form_factor(static_cast<form_factor_t>(ff1));
            const FormFactor& ff2_obj = storage::get_form_factor(static_cast<form_factor_t>(ff2));
...
TEST_CASE("PrecalculatedFormFactorProduct::table") {
    const auto& table = storage::get_precalculated_form_factor_table();
    for (unsigned int ff1 = 0; ff1 < get_count(); ++ff1) {
        for (unsigned int ff2 = 0; ff2 < get_count(); ++ff2) {
            const FormFactor& ff1_obj = storage::get_form_factor(static_cast<form_factor_t>(ff1));
            const FormFactor& ff2_obj = storage::get_form_factor(static_cast<form_factor_t>(ff2));
