...
Usage:
    # Run the benchmarks (from the project root, so the tests/files/ paths resolve):
    cmake --build build --target benchmarks -j8
    ./build/tests/benchmark/bin/benchmark_pr "[benchmark]~[large]" -r xml::out=tests/benchmark/results.xml

    # For a faster run with fewer samples:
    ./build/tests/benchmark/bin/benchmark_pr "[benchmark]~[large]" --benchmark-samples 10 -r xml::out=tests/benchmark/results.xml
...
def parse_xml(path: str) -> dict:
    ...
    for tc in root.iter("TestCase"):
        name = tc.get("name", "")
        is_basic     = name.startswith("Distance calculation benchmark")
        is_intensity = name.startswith("Intensity calculation benchmark")
        is_partial   = "rigidbody" in name or "Symmetry histogram" in name

        for section in tc.iter("Section"):
            sec_name = section.get("name", "")

            if is_basic or is_intensity:
                target = basic if is_basic else intensity
                ...
                for br in section.findall("BenchmarkResults"):
                    ...
                    target.setdefault(bench_name, []).append((atoms, mean_ms, std_ms))
    ...
    # sort the size-sweep series by atom count
    for series in (basic, intensity):
        for k in series:
            series[k].sort(key=lambda x: x[0])

    return {"basic": basic, "intensity": intensity, "partial": partial}
...
COLOURS = {
    ...
    "Fraser":              "tab:green",
    "Grid":                "tab:red",
    "Fraser (sinqd warm)": "tab:green",
    "Fraser (sinqd cold)": "tab:olive",
    "Grid (sinqd warm)":   "tab:red",
    "Grid (sinqd cold)":   "tab:pink",
...
def main():
    ...
    n_plots = int(bool(data["basic"])) + int(bool(data["intensity"])) + int(bool(data["partial"]))
    if n_plots == 0:
    ...
    if data["intensity"]:
        plot_basic(axes[0][col], data["intensity"],
                   "Intensity: debye_transform only")
        col += 1
