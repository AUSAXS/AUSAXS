...
TEST_CASE("Molecule::simulate_dataset", "[files]") {
    ...
    // the dataset is generated by drawing random noise, so chi2/dof is itself a random variable with
    // sigma = sqrt(2/dof) ~ 0.16 here. the tolerance below is only ~3 sigma wide, so an unseeded run
    // fails roughly once every 350 times. fix the seed to make the outcome deterministic instead.
    random::set_seed(69);
