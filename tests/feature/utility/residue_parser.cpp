...
TEST_CASE("ResidueParser: cif_reader_single") {
    if (auto folder = io::Folder("temp/residues_single"); !folder.exists()) {
        folder.create();
    }

    io::File gly("temp/residues_single/GLY.cif");
    if (!gly.exists()) {
        REQUIRE(ausaxs::curl::download("files.rcsb.org/ligands/view/GLY.cif", gly.str()));
    }
...
TEST_CASE("ResidueParser: cif_reader_all") {
    if (auto folder = io::Folder("temp/residues_all"); !folder.exists()) {
        folder.create();
    }

    for (const auto& [acid, atom_map] : hydrogen_atoms::get) {
        io::File res("temp/residues_all/" + acid + ".cif");
        if (!res.exists()) {
        ...
        for (const auto& [atom, num_hydrogens] : atom_map) {
            INFO(acid + " " + atom);
            CHECK(num_hydrogens == residue.get(atom, constants::symbols::parse_element_string(std::string(1, atom[0]))));
        }
    }
}