...
TEST_CASE("ExvFormFactor::plot", "[manual]") {
    ...
    {
        const form_factor::NormalizedFormFactor& ff_exv = form_factor::storage::atomic::get_form_factor(form_factor::form_factor_t::EXCLUDED_VOLUME);
...
TEST_CASE("ExvFormFactor::plot_cmp", "[manual]") {
    for (unsigned int ffi = 0; ffi < form_factor::get_count_without_excluded_volume(); ++ffi) {
        const form_factor::NormalizedFormFactor& ff = form_factor::storage::atomic::get_form_factor(static_cast<form_factor::form_factor_t>(ffi));
