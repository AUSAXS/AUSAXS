...
TEST_CASE("Slice::copy to slice") {
    Matrix<double> B = {{1, 2, 2, 7}, {9, 5, 2, 1}, {6, 1, 1, 3}};
    auto a = B.row(1);
...
TEST_CASE("Slice::operator Vector") {
    ...
    REQUIRE((A.col(2).operator Vector<double>().size() == 3 && A.col(2).operator Vector<double>().data.size() == 3));
}

TEST_CASE("Slice::operator vector") {
    Matrix<double> A = {{1, 2, 2, 7}, {9, 5, 2, 1}, {6, 1, 1, 3}};
    std::vector<double> a = A.col(2);
    REQUIRE(a == std::vector<double>{2, 2, 1});
    REQUIRE((A.col(2).operator std::vector<double>() == std::vector<double>{2, 2, 1}));
}

TEST_CASE("Slice::dot") {
    Matrix<double> A = {{1, 2, 2, 7}, {9, 5, 2, 1}, {6, 1, 1, 3}};

    SECTION("dot with vector") {
