...
TEST_CASE("NamedDataset::NamedDataset") {
    SECTION("default constructor") {
        NamedDataset<Dataset> dataset;
        CHECK(dataset.dataset.empty());
        CHECK(dataset.names.empty());
    }

    SECTION("from Dataset with default names") {
        Dataset d(10, 3);
        NamedDataset<Dataset> named(std::move(d));
        CHECK(named.dataset.size_rows() == 10);
        CHECK(named.dataset.size_cols() == 3);
    ...
    SECTION("from Dataset with explicit names") {
        ...
        NamedDataset<Dataset> named(std::move(d), names);
        CHECK(named.dataset.size_rows() == 10);
        CHECK(named.dataset.size_cols() == 3);
    ...
    SECTION("from SimpleDataset") {
        ...
        NamedDataset<SimpleDataset> named(std::move(sd), {"q", "I", "I_err"});
        CHECK(named.dataset.size() == 3);
...
TEST_CASE("NamedDataset::is_named") {
    ...
    SECTION("custom names") {
        Dataset d(10, 3);
        NamedDataset<Dataset> named(std::move(d), {"x", "y", "z"});
...
TEST_CASE("NamedDataset::select_columns") {
    ...
    SECTION("select by name") {
        ...
        CHECK(selected.dataset.size_cols() == 2);
        CHECK(selected.names == std::vector<std::string>{"x", "z"});
        CHECK(selected.dataset.col(0)[0] == 1.0);
        CHECK(selected.dataset.col(1)[0] == 7.0);
    }
