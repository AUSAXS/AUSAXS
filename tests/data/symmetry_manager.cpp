...
TEST_CASE("SymmetryManager::calculate") {
    ...
    SECTION("real data") {
        ...
        data::detail::Symmetry s{{20, 0, 0}, {0, 0, 0}}; 
        SECTION("single copy") {
            ...
            CHECK(compare_hist(h->get_aa_counts(), h2->get_aa_counts()));
            CHECK(compare_hist(h->get_ww_counts(), h2->get_ww_counts()));
            CHECK(compare_hist(h->get_aw_counts(), h2->get_aw_counts()));
            CHECK(compare_hist(h->get_total_counts(), h2->get_total_counts()));
        }
    }
}