...
int main(int argc, char *argv[]){
  ...
  for (unsigned int i=0; i< pdb.number_of_models(); ++i){
    const dsrpdb::Model &m= pdb.model(i);
	...
	const dsrpdb::Protein &p= m.chain(j);
	...
	if (split_chain != '\0') {
	  ...
	  dsrpdb::Protein ps[2]={dsrpdb::Protein(), dsrpdb::Protein()};
	  for (dsrpdb::Protein::Const_residues_iterator rit = p.residues_begin(); 
	       rit != p.residues_end(); ++rit){
	    //rit->write('c', std::cout);
	    if (static_cast<unsigned int>(rit->index())
