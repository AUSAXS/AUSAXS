...
void ScatteringHistogram::setup() {
    ...
    for (int i = 0; i < debye_axes.bins; i++) {
...
void ScatteringHistogram::apply_water_scaling_factor(const double& k) {
    double k2 = pow(k, 2);
    for (int i = 0; i < axes.bins; i++) {p[i] = p_pp[i] + k*p_hp[i] + k2*p_hh[i];} // p = p_tot, inherited from Histogram
}
...
vector<double> ScatteringHistogram::calc_debye_scattering_intensity() const {
    ...
    for (int i = 0; i < debye_axes.bins; i++) { // iterate through all q values
