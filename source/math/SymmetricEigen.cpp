...
namespace ausaxs::matrix {
    // Cyclic Jacobi rotation method: robust and accurate for the small dense symmetric matrices this is used with (up to 9x9).
    EigenResult symmetric_eigen(const Matrix<double>& A) {
        ...
        auto vt = [&](int i, int j) -> double& {return v[i*n + j];};
        ...
        std::ranges::sort(order, [&](int i, int j) {return at(i, i) < at(j, j);});
        ...
        for (int idx : order) {
            ...
            for (int k = 0; k < n; ++k) {vec[k] = vt(k, idx);}
