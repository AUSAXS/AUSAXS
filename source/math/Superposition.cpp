...
namespace ausaxs::matrix {
    // Orthogonal Procrustes solution (the rotation maximizing the Frobenius product <R, H>), computed via Horn's quaternion method: the optimal rotation 
    // is the largest eigenvector of a symmetric 4x4 matrix built from H. Going through a quaternion guarantees a proper rotation, never a reflection.
    ...
    // Kabsch algorithm: centre both point sets, then recover the optimal rotation from their cross-covariance and the translation that lines the centroids up.
