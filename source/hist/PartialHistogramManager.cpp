...
void PartialHistogramManager::calc_self_correlation(unsigned int index) {
    ...
    //     threads.push_back(std::thread([this, &current, &p_pp, &width, &lock, index, i]() 
    //     {
    //         std::vector<double> p_pp_temp(p_pp.size(), 0);
    ...
    //                 float dist = sqrt(dx*dx + dy*dy + dz*dz);
    //                 p_pp_temp[dist/width] += 2*weight;
    //             }
    //         }
    //         std::lock_guard<std::mutex> guard(lock);
    //         for (unsigned int j = 0; j < p_pp.size(); j++) {p_pp[j] += p_pp_temp[j];}
    //     }));
    // }
...
ScatteringHistogram PartialHistogramManager::calculate_slow_mt() const {
    auto atoms = protein->atoms();
    auto waters = protein->waters();
