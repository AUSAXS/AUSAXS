    ...
    if (min_spacing > 0) {
        ...
        for (int i : local_minima) {
            if (filtered_minima.empty()) {
                ...
                if (x(i) - x(filtered_minima.back()) > min_spacing) {
                    filtered_minima.push_back(i);
                }
            }
        }
    ...
    local_minima_bounds.reserve(local_minima.size());
    {
        ...
        if (local_minima_bounds.size() > 1) {
            ...
            for (unsigned int i = 0; i < local_minima_bounds.size()-1; i++) {
                ...
                while (i+merge_count < local_minima_bounds.size()-1) {
                    if (bounds.max <= local_minima_bounds[i+merge_count].min) {break;}
                    bounds.max = local_minima_bounds[i+merge_count].max;
                    merge_count++;
                    std::cout << "Merging bounds (" << bounds.min << ", " << bounds.max << ") and (" 
                        << local_minima_bounds[i+merge_count].min << ", " << local_minima_bounds[i+merge_count].max << "). New bounds: (" << bounds.min << ", " << bounds.max << ")" << std::endl;
                }
            ...
            if (!merged_bounds.empty()) {
                for (auto b : merged_bounds) {
                    SimpleDataset dummy({x(b.min), x(b.max)}, {y(b.min), y(b.max)});
                    dummy.add_plot_options({{"color", style::color::red}, {"ls", style::line::dashed}});
                    plot.plot(dummy);
                }
        ...
        if (min_prominence > 0) {
            ...
            for (unsigned int i = 0; i < local_minima.size(); i++) {
                ...
                double x1 = x(local_minima_bounds[i].min);
                double x2 = x(local_minima_bounds[i].max);
                double y1 = y(local_minima_bounds[i].min);
                double y2 = y(local_minima_bounds[i].max);
                double a = (y2-y1)/(x2-x1);
                double b = y1 - a*x1;
                double prominence = std::abs(y(local_minima[i]) - (a*x(local_minima[i]) + b));

                SimpleDataset dummy({x(local_minima[i]), x(local_minima[i])}, {y(local_minima[i]), y(local_minima[i]) + prominence});
                dummy.add_plot_options({{"color", style::color::red}, {"ls", style::line::dashed}});
                plot.plot(dummy);
