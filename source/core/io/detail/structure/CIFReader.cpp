...
struct CIFSection {
    ...
    std::unordered_map<std::string, int> get_label_map() const {
        ...
        for (size_t i = 0; i < labels.size(); i++) {
            map[labels[i]] = static_cast<int>(i);
        }
        return map;
    }
};
...
namespace {
    residue::detail::Residue parse_ion(const CIFSection& atom) {
        ...
        if (!labels.contains("comp_id")) {
            throw except::io_error(R"(CIFReader::parse_ion: Missing required label "comp_id" in "_chem_comp" section)");
        } if (!labels.contains("charge")) {
            throw except::io_error(R"(CIFReader::parse_ion: Missing required label "charge" in "_chem_comp" section)");
        }
    ...
    std::vector<residue::detail::Residue> parse_residue(CIFSection& atom, CIFSection& bond) {
                ...
                while (i < atom.data.size() && atom.data[i][i_comp_id] == current_comp_id) {
                    ...
                    if (atom_id == alt_atom_id) {
                        if (auto N = atom_id.size(); N > 2) { // second character must be a locator e.g. A, B, C, D, E, ...
                            if (atom_id[N - 1] == '1' && !utility::isdigit(atom_id[N - 2])) { // last character must be 1 & previous character must be a locator
        ...
        for (size_t i = 0; i < bond.data.size();) { // nested loops are responsible for incrementing i
            ...
            while (i < bond.data.size() && bond.data[i][i_comp_id_bond] == current_comp_id) {
                ...
                int value_order = residue::detail::Bond::parse_order(bond.data[i][i_value_order]);
    ...
    void parse_atom_site_section(CIFSection& atom, io::pdb::PDBStructure& collection, bool allow_fractional_coords = false) {
        ...
        for (auto& data : atom.data) {
            ...
            // load mandatory data
            try {
                name = has_atom_name ? data[i_label_atom_id] : data[i_type_symbol];
                resName = default_to_UNK.empty() ? data[i_label_comp_id] : default_to_UNK;
                coords = {
                    std::stod(data[i_coord_x]), 
                    std::stod(data[i_coord_y]), 
                    std::stod(data[i_coord_z]),
                };
                element = constants::symbols::parse_element_string(data[i_type_symbol]);
                if (has_alt_loc)     {altLoc   = data[i_label_alt_id];}
                if (has_chainID)     {chainID  = data[i_label_asym_id][0];}
                if (has_ins_code)    {iCode    = data[i_PDB_ins_code];}
                if (has_charge)      {charge   = data[i_pdbx_formal_charge];}
                if (has_serial      && !data[i_id].starts_with('.'))            {serial     = std::stoi(data[i_id]);}
                if (has_resSeq      && !data[i_label_seq_id].starts_with('.'))  {resSeq     = std::stoi(data[i_label_seq_id]);}
                if (has_occupancy   && !data[i_occupancy].starts_with('.'))     {occupancy  = std::stod(data[i_occupancy]);}
                if (has_temp_factor && !data[i_B_iso_or_equiv].starts_with('.')) {tempFactor = std::stod(data[i_B_iso_or_equiv]);}
        ...
        if (!collection.atoms.empty() && std::ranges::all_of(collection.atoms, [] (const auto& a) {return a.occupancy == 0;})) {
    ...
    */
    CIFSection extract_section(std::string_view pattern, std::string_view first_match, std::ifstream& input) {
        ...
        data.emplace_back(); // add an empty data entry in case the labels also contains data
        do {
            ...
            if (!line.empty() && line.starts_with('_')) { // label line
                ...
                if (!tokens[0].starts_with(pattern)) {
                    ...
                    return CIFSection{.labels=std::move(labels), .data=std::move(data)};
                }
        ...
        return {.labels=std::move(labels), .data=std::move(data)};
    }
}
...
namespace {
    io::pdb::PDBStructure read_pdbx(CIFSection&& chem_comp_atom, CIFSection&& chem_comp_bond, CIFSection&& atom_site, const std::string& path) { // NOLINT
        ...
        int n_pa = static_cast<int>(file.atoms.size());
        int n_ha = static_cast<int>(file.waters.size());
    ...
    io::pdb::PDBStructure read_crystal(CIFSection&& cell_, CIFSection&& atom_site, const std::string& /*unused*/) { // NOLINT
        io::pdb::PDBStructure file;
