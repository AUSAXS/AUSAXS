...
FitResult::FitResult(const mini::Result& res, int dof) noexcept : Result(res), dof(dof) {}
...
void FitResult::add_fit(observer_ptr<FitResult> fit, bool front) noexcept {
    ...
    dof -= static_cast<int>(fit->parameters.size());
}
...
void FitResult::set_data_curves(NamedDataset&& curves_) {
    ...
    curves.set_col_names({"q", "I", "I_err", "I_fit", "residuals"});
}
