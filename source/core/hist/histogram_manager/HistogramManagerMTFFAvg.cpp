...
namespace {
    ...
    void evaluate_aa8(WeightedDistribution3D& p, const CompactCoordinatesFF<vbw>& data_a, int i, int j) {
        ...
        for (int k = 0; k < 8; ++k) {
            p.increment_linear<factor>(res.ff_bins[k], res.distance_bins[k], res.distances[k]);
            p.increment_bin<factor>(ff_i, form_factor::exv_bin, res.distance_bins[k], res.distances[k]);
            p.increment_bin<factor>(form_factor::exv_bin, form_factor::exv_bin, res.distance_bins[k], res.distances[k]);
        }
    }
    ...
    void evaluate_aa8(Distribution3D& p, const CompactCoordinatesFF<vbw>& data_a, int i, int j) {
        ...
        int ff_i = data_a.get_ff_type(i);
        for (int k = 0; k < 8; ++k) {
            p.increment_linear_bin<factor>(res.ff_bins[k], res.distances[k]);
    ...
    void evaluate_aa1(WeightedDistribution3D& p, const CompactCoordinatesFF<vbw>& data_a, int i, int j) {
        ...
        p.increment_bin<factor>(ff_i, ff_j, res.distance_bin, res.distance);
        p.increment_bin<factor>(ff_i, form_factor::exv_bin, res.distance_bin, res.distance);
        p.increment_bin<factor>(form_factor::exv_bin, form_factor::exv_bin, res.distance_bin, res.distance);
    }
    ...
    void evaluate_aw8(WeightedDistribution2D& p, const CompactCoordinatesFF<vbw>& data_a, const CompactCoordinatesFF<vbw>& data_w, int i, int j) {
        ...
        int ff_i = data_a.get_ff_type(i);
        for (int k = 0; k < 8; ++k) {
            p.increment_bin<factor>(ff_i, res.distance_bins[k], res.distances[k]);
            p.increment_bin<factor>(form_factor::exv_bin, res.distance_bins[k], res.distances[k]);
        }
    }
    ...
    void evaluate_aw1(WeightedDistribution2D& p, const CompactCoordinatesFF<vbw>& data_a, const CompactCoordinatesFF<vbw>& data_w, int i, int j) {
        ...
        int ff_i = data_a.get_ff_type(i);
        p.increment_bin<factor>(ff_i, res.distance_bin, res.distance);
        p.increment_bin<factor>(form_factor::exv_bin, res.distance_bin, res.distance);
    }
