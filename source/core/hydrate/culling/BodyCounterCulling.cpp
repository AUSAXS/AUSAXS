...
void hydrate::BodyCounterCulling::cull(std::span<grid::GridMember<data::Water>>& placed_water) const {
    if (body_ratios.empty()) {CounterCulling(molecule).cull(placed_water); return;}
    double total_reduction_factor = static_cast<double>(placed_water.size())/target_count;
    double total_weight = std::reduce(body_ratios.begin(), body_ratios.end(), 0.0);
    ...
    std::vector<int> body_counts(molecule->size_body(), 0);
    std::vector<double> body_counters(molecule->size_body(), 0);
    int rm_index = 0; // current index in removed_water
    int fw_index = 0; // current index in final_waters
    for (const auto& water : placed_water) {
        const Vector3<double>& pos = water.get_absolute_loc();
        double min_dist = std::numeric_limits<double>::max();
        int min_index = 0;
        for (int i = 0; i < static_cast<int>(cms.size()); ++i) {
