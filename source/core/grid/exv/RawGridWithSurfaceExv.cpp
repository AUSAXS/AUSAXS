...
namespace {
    bool collision_check(observer_ptr<grid::Grid> grid, const Vector3<int>& loc) {
        static double width = grid::Grid::get_width();
        static grid::detail::RadialLineGenerator radial_lines({std::sqrt(grid::Grid::get_width())+1e-3, 2*grid::Grid::get_width(), 3*grid::Grid::get_width(), 4*grid::Grid::get_width()});
        if (width != grid::Grid::get_width()) {
            width = grid::Grid::get_width();
            radial_lines = grid::detail::RadialLineGenerator({std::sqrt(grid::Grid::get_width())+1e-3, 2*grid::Grid::get_width(), 3*grid::Grid::get_width(), 4*grid::Grid::get_width()});
        }
    ...
    GridExcludedVolume helper(observer_ptr<grid::Grid> grid) {
        ...
        vol.interior_sites.reserve(static_cast<int>(grid->get_volume()));
        ...
        auto add_surface = [&vol, &grid, &start, stride] (int i, int j, int k) {
            ...
            for (int j = start.y(); j < end.y(); j+=stride) {
                for (int k = start.z(); k < end.z(); k+=stride) {
                    ...
                    if constexpr (!unity_width) {
                        ...
                        if (collision) {add_interior(i, j, k);}
                        else           {add_surface(i, j, k);}
                    }
                }
            }
        }
        ...
        if constexpr (!unity_width) {
            int expand = static_cast<int>(std::round(settings::grid::exv::surface_thickness/settings::grid::cell_width)*0.5);
