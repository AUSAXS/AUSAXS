...
ReferenceSymmetryView::ReferenceSymmetryView(observer_ptr<ReferenceSymmetry> target) : target(target) {
    assert(target != nullptr && "ReferenceSymmetryView: target cannot be null.");
}
...
std::unique_ptr<ISymmetry> ReferenceSymmetryView::clone() const {
    return std::make_unique<ReferenceSymmetryView>(target);
}
