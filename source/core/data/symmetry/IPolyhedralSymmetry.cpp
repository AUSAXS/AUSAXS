...
namespace {
    ...
    std::vector<Matrix<double>> close_group(const std::vector<Matrix<double>>& generators, int expected_order) {
        ...
        for (int i = 0; i < static_cast<int>(elements.size()) && static_cast<int>(elements.size()) <= expected_order; ++i) {
...
int IPolyhedralSymmetry::repetitions() const {
    return static_cast<int>(group().elements.size()) - 1;
}
...
AffineTransform IPolyhedralSymmetry::_make_transform(const Vector3<double>& anchor, int rep) const {
    ...
    Vector3<double> c = group_centre(anchor, F);
    Vector3<double> T = c - R*c;
    return {.rotation=std::move(R), .translation=T};
}

Vector3<double> IPolyhedralSymmetry::group_centre(const Vector3<double>& cm, const Matrix<double>& /*unused*/) const {
    return cm + translation;
}

std::span<double> IPolyhedralSymmetry::span_translation() {return {translation.begin(), translation.end()};}
std::span<double> IPolyhedralSymmetry::span_rotation() {return {rotation.begin(), rotation.end()};}
...
ISymmetry& IPolyhedralSymmetry::add(observer_ptr<const ISymmetry> other) {
    const auto* cast = dynamic_cast<const IPolyhedralSymmetry*>(other);
