...
IDistanceConstraint::IDistanceConstraint(
    restore_t, observer_ptr<const Molecule> molecule, int ibody1, int iatom1, int ibody2, int iatom2,
    std::pair<int, int> isym1, std::pair<int, int> isym2, double d_target
) : d_target(d_target), molecule(molecule), ibody1(ibody1), ibody2(ibody2), iatom1(iatom1), iatom2(iatom2),
    isym1(std::move(isym1)), isym2(std::move(isym2))
{}
