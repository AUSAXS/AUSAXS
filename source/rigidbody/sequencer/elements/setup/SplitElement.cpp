...
SplitElement::SplitElement(observer_ptr<Sequencer> owner, const std::string& body_name, const std::vector<int>& splits) : owner(owner) {
...
void SplitElement::_split(const std::string& body_name, const std::vector<int>& splits) {
    ...
    for (int i = 0; i < original.size_symmetry(); ++i) {
        if (dynamic_cast<const symmetry::ReferenceSymmetryView*>(original.symmetry().get(i)) != nullptr) {
    ...
    for (int b = 0; b < molecule->size_body(); ++b) {
        if (b == ib) {continue;}
        const auto& other = molecule->get_body(b);
        for (int i = 0; i < other.size_symmetry(); ++i) {
            const auto* view = dynamic_cast<const symmetry::ReferenceSymmetryView*>(other.symmetry().get(i));
    ...
    int append_at = molecule->size_body();
    ...
    for (int k = 0; k < static_cast<int>(fragments.size()); ++k) {final_indices[k] = append_at + k - 1;}

    name_map.add_body(final_indices.front(), inherited);
    for (int k = 1; k < static_cast<int>(final_indices.size()); ++k) {name_map.add_body(final_indices[k]);} // the trailing fragments are new bodies, with fresh names
    ...
    for (int i = 0; i < static_cast<int>(orig_symmetries.size()); ++i) {
        molecule->get_body(primary).symmetry().add(std::make_unique<symmetry::ReferenceSymmetry>(
            orig_symmetries[i]->clone(), final_indices, std::vector<int>(final_indices.size(), i), molecule
        ));

        auto* ref = static_cast<symmetry::ReferenceSymmetry*>(molecule->get_body(primary).symmetry().get(i));
        int reps = ref->repetitions();

        for (int k = 1; k < static_cast<int>(final_indices.size()); ++k) {
            int b = final_indices[k];
            molecule->get_body(b).symmetry().add(std::make_unique<symmetry::ReferenceSymmetryView>(molecule, primary, i));
        }

        for (int b : final_indices) {
            for (int j = 0; j < reps; ++j) {name_map.add_replica(b, i, j+1);}
        }
    }
    ...
    for (int idx : final_indices) {
        ...
        for (int j = 0; j < snapshot.size_symmetry(); ++j) {
...
std::unique_ptr<GenericElement> SplitElement::_parse(observer_ptr<LoopElement> owner, ParsedArgs&& args) { // NOLINT
    std::string body_name = args.inlined[0];
    std::vector<int> splits;
    splits.reserve(args.inlined.size()-1);
    for (int i = 1; i < static_cast<int>(args.inlined.size()); ++i) {
