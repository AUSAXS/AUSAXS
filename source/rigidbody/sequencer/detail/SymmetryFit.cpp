...
namespace {
    ...
    Vector3<double> solve_common_centre(const std::vector<Matrix<double>>& R, const std::vector<Vector3<double>>& t, const Vector3<double>& cm) {
        ...
        for (int d = 0; d < 3; ++d) {normal(d, d) += lambda;}
        rhs += lambda*cm;
    ...
    Matrix<double> solve_frame(const std::vector<Matrix<double>>& R, const std::vector<Matrix<double>>& G) {
        ...
        for (std::size_t k = 1; k < R.size(); ++k) { // element 0 is the identity: L is zero
            ...
            for (int a = 0; a < 3; ++a) {
                for (int b = 0; b < 3; ++b) {
                    int row = 3*a + b;
                    for (int i = 0; i < 3; ++i) {L(row, 3*i + b) += R[k](a, i);}   //  sum_i R(a,i) F(i,b)
                    for (int j = 0; j < 3; ++j) {L(row, 3*a + j) -= G[k](j, b);}   // -sum_j F(a,j) G(j,b)
                }
            }
            Q += L.transpose()*L;
        }
    ...
    bool is_trivial_cyclic(const symmetry::ISymmetry& sym) {
        const auto* cs = dynamic_cast<const symmetry::CyclicSymmetry*>(&sym);
        return (cs != nullptr)
...
SymmetryFitResult fit_symmetry(
    ...
    assert(static_cast<int>(copies.size()) == template_symmetry.repetitions() + 1 && "fit_symmetry: body count must equal repetitions()+1.");
    ...
    if (const auto* comp = dynamic_cast<const symmetry::CompositeSymmetry*>(&template_symmetry)) {
        int stride = 1 + comp->inner->repetitions();
        int outer_reps = comp->outer->repetitions();
...
SymmetryFitResult fit_symmetry_best_order(
    ...
    while (std::ranges::next_permutation(perm).found) { // the identity order was already tried
