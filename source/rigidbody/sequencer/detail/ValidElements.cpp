...
namespace {
    std::string quote_join(std::vector<std::string> values) {
        std::ranges::sort(values); // the keys arrive from an unordered_map, so sort for a reproducible message
        std::string joined;
    ...
    const std::map<ElementType, std::vector<std::string>>& get_type_map() {
        static std::map<ElementType, std::vector<std::string>> type_map = {
            ...
            {ElementType::Settings, {"settings", "setting"}},
...
std::vector<std::string> ausaxs::rigidbody::sequencer::detail::valid_arguments(ElementType type) {
    switch (type) {
        ...
        case ElementType::Settings:            return SettingsElement::_valid_arguments();
...
ausaxs::rigidbody::sequencer::InlineSignature ausaxs::rigidbody::sequencer::detail::valid_inline_arguments(ElementType type) {
    switch (type) {
        ...
        case ElementType::Settings:            return SettingsElement::_valid_inline_arguments();
...
void ausaxs::rigidbody::sequencer::detail::validate_named_arguments(ElementType type, std::string_view element, const ParsedArgs& args) {
    ...
    for (const auto& [key, value] : args.named) {
        if (std::ranges::find(valid, key) != valid.end()) {continue;}
    ...
    msg += valid.empty()
        ...
        : ". Valid arguments are: " + quote_join(valid) + ".";
    throw except::parse_error(element, msg);
}
...
ElementType ausaxs::rigidbody::sequencer::detail::get_type(std::string_view line) {
    ...
    for (const auto& [type, prefixes] : type_map) {
        for (const auto& prefix : prefixes) {
            if (line.starts_with(prefix) && (line.size() == prefix.size() || !utility::isalpha(line[prefix.size()]))) {
                return type;
            }
        }
    }
