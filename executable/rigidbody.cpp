...
int main(int argc, char const *argv[]) { 
    ...
    auto p_cal = app.add_option("--calibrate", setting::rigidbody::detail::calibration_file, "Path to the calibration data.")->expected(0, 1)->check(CLI::ExistingFile);
    ...
    app.add_option("--iterations", setting::rigidbody::iterations, "Maximum number of iterations. Default: 1000.");
    ...
    if (p_cal->count() != 0) {
            ...
            for (const auto& name : valid_names) {
                for (const auto& ext : constants::filetypes::saxs_data.extensions) {
                    ...
                    if (std::filesystem::exists(path)) {
                        setting::rigidbody::detail::calibration_file = path;
        ...
        setting::general::output += "calibrated/";
        ...
        setting::general::output += "uncalibrated/";
    }
