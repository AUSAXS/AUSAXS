...
class Ligand {
    class Atom {
        public: 
            Atom(string symbol, unsigned int id) : symbol(symbol), id(id) {
                valence = constants::valence::get.at(symbol);
            }

            void add_connection(Ligand::Atom* atom) {
                // std::cout << "\nAdd connection" << std::endl;
            ...
            std::string to_string() const {
                return "Atom " + std::to_string(id) + " \"" + symbol + "\" named \"" + name + "\": " + std::to_string(valence);
            }

            friend std::ostream& operator<<(std::ostream& os, const Ligand::Atom& a) {os << a.to_string(); return os;}
        
            std::string symbol, name;
            unsigned int valence;
            std::vector<Ligand::Atom*> connections;
            unsigned int id;
    };
    ...
    public: 
        ...
        Ligand::Atom* find_calpha() {
            for (unsigned int i = 0; i < structure.size(); i++) {
                ...
                if (atom1.symbol == "O") {
                    std::cout << "Found oxygen (id " << atom1.id << ")" << std::endl;
                    ...
                    for (unsigned int j = 0; j < atom1.connections.size(); j++) {
                        ...
                        if (atom2->symbol == "C") {
                            std::cout << "Found attached carbon (id " << atom2->id << ")" << std::endl;
        ...
        std::map<unsigned int, Ligand::Atom> parse_initial(std::string id, unsigned int& index) {
            ...
            while(index < id.size()) {
                ...
                for (int i = 0; i < std::stoi(number); i++) {
                    atoms.emplace(counter, Ligand::Atom(symbol, counter));
                    counter++;
                }
            }
            ...
            while(index < id.size()) {
                ...
                while(index < id.size()) {
                    ...
                    // afterwards we always make at least one connection
                        ...
                        // the numbers 'previous' and 'current' are atom ids as read from the InChI string, but we use zero-indexing so we have to subtract 1
                        structure.at(previous-1).add_connection(&structure.at(current-1));
...
int main(int, char const**) {
    string test = "InChI=1S/C21H19ClN6O/c22-19-13-24-21-20(26-18(14-28(19)21)15-2-1-7-23-12-15)25-16-3-5-17(6-4-16)27-8-10-29-11-9-27/h1-7,12-14H,8-11H2,(H,25,26)"; // ASH
    Ligand ligand(test);
    // ligand.print();
    ligand.enumerate();
    return 0;
