...
class Matrix {
  ...
  public: 
     ...
     */
    Matrix(Matrix<Q>&& A) noexcept : _N(A.N), _M(A.M), _data(std::move(A._data)) {}

    /**
     * @brief Copy constructor.
     */
    Matrix(const Matrix<Q>& A) : _N(A.N), _M(A.M), _data(A.data) {}

    /**
     * @brief Construct a Matrix based on a nested initializer list. The lists must be of the same size. 
     */
    Matrix(std::initializer_list<std::initializer_list<Q>> l) : _N(l.size()), _M(l.begin()->size()) {
     ...
     */
    Matrix(const Vector<Q>& v) : _N(v.N), _M(1), _data(v.data) {}
    ...
    // Read-only indexer
    const Q& index(int i, int j) const {return data[M*i + j];}

    // Read-write indexer
    Q& index(int i, int j) {return _data[M*i + j];}
    ...
    // Read-write iterator
    typename std::vector<Q>::iterator begin() {return _data.begin();}

    // Read-write iterator
    typename std::vector<Q>::iterator end() {return _data.end();}
    ...
    const std::vector<Q>& data = _data; // read-only access to the data container
