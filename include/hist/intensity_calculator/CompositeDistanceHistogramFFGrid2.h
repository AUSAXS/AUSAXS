...
namespace hist {
    class CompositeDistanceHistogramFFGrid2 : public CompositeDistanceHistogramFFGrid {
        public:
            using CompositeDistanceHistogramFFGrid::CompositeDistanceHistogramFFGrid;
            ~CompositeDistanceHistogramFFGrid2() override = default;
        ...
        private:
            virtual double exv_factor(double q) const override {
                ...
                return std::clamp(std::exp(-c*(cx*cx - 1)*q*q), 0., 10.);
            }
    };
}