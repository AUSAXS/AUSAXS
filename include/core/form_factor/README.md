...
- `ExvTable.h` — `exv_info_table`, the optional excluded-volume descriptor table with one row per type and one column per displaced-volume set. A type missing from the current set cannot be used with the Fraser model and is treated as OTHER.
