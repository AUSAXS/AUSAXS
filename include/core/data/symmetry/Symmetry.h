...
namespace ausaxs::symmetry {
    class Symmetry {
        ...
        public:
            ...
            Symmetry(_Relation initial_relation, int repetitions = 1)
                : initial_relation(initial_relation), repetitions(repetitions) 
            {assert(sanity_checks());}
    ...
    inline bool Symmetry::sanity_checks() const {
        ...
        assert(1e-9 < repeat_relation.axis.magnitude() && "Rotation axis cannot be zero vector.");
        ...
        assert([&]() {
            ...
            return repeat_relation.angle < 1e-9 || R_r*repeat_relation.translation == repeat_relation.translation;
...
inline bool ausaxs::symmetry::Symmetry::is_closed() const {
    ...
    constexpr int cmp = 314; // 100*2pi rounded to nearest integer
