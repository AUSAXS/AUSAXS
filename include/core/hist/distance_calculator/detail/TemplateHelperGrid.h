...
namespace ausaxs::hist::detail {
    ...
    inline void evaluate16(WeightedDistribution1D& p, const CompactCoordinatesFF<variable_bin_widths>& data_i, const CompactCoordinates<variable_bin_widths>& data_j, int i, int j) {
    ...
    inline void evaluate4(WeightedDistribution1D& p, const CompactCoordinatesFF<variable_bin_widths>& data_i, const CompactCoordinates<variable_bin_widths>& data_j, int i, int j) {
        ...
        for (int k = 0; k < 4; ++k) {p.increment_index<factor>(res.distance_bins[k], res.distances[k]);}
    }
    ...
    inline void evaluate1(WeightedDistribution1D& p, const CompactCoordinatesFF<variable_bin_widths>& data_i, const CompactCoordinates<variable_bin_widths>& data_j, int i, int j) {
        ...
        p.increment_index<factor>(res.distance_bin, res.distance);
    }

    // Unweighted distribution overloads
    template<bool variable_bin_widths, int factor = 1>
    inline void evaluate16(Distribution1D& p, const CompactCoordinatesFF<variable_bin_widths>& data_i, const CompactCoordinates<variable_bin_widths>& data_j, int i, int j) {
        xyzff::HexaEvaluatedResultRounded res = add16::evaluate_unweighted(data_i, data_j, i, j);
        for (int k = 0; k < 16; ++k) {p.increment_index<factor>(res.distance_bins[k]);}
    }
    ...
    inline void evaluate8(Distribution1D& p, const CompactCoordinatesFF<variable_bin_widths>& data_i, const CompactCoordinates<variable_bin_widths>& data_j, int i, int j) {
        ...
        for (int k = 0; k < 8; ++k) {p.increment_index<factor>(res.distance_bins[k]);}
    }
    ...
    inline void evaluate4(Distribution1D& p, const CompactCoordinatesFF<variable_bin_widths>& data_i, const CompactCoordinates<variable_bin_widths>& data_j, int i, int j) {
        ...
        for (int k = 0; k < 4; ++k) {p.increment_index<factor>(res.distance_bins[k]);}
    }
...
namespace ausaxs::hist::detail::grid {
    ...
    template<bool variable_bin_width, int factor = 1>
    inline void evaluate16(
        ...
        xyzff::HexaEvaluatedResult res = add16::evaluate_weighted(data_i, data_j, i, j);
        for (int k = 0; k < 16; ++k) {p.increment_index<factor>(data_i.get_ff_type(i), res.distance_bins[k], res.distances[k]);}
    }
    ...
    inline void evaluate8(
        ...
        xyzff::OctoEvaluatedResult res = add8::evaluate_weighted(data_i, data_j, i, j);
        for (int k = 0; k < 8; ++k) {p.increment_index<factor>(data_i.get_ff_type(i), res.distance_bins[k], res.distances[k]);}
    }
    ...
    inline void evaluate4(
        ...
        xyzff::QuadEvaluatedResult res = add4::evaluate_weighted(data_i, data_j, i, j);
        for (int k = 0; k < 4; ++k) {p.increment_index<factor>(data_i.get_ff_type(i), res.distance_bins[k], res.distances[k]);}
    }
    ...
    inline void evaluate1(
        ...
        xyzff::EvaluatedResult res = add1::evaluate_weighted(data_i, data_j, i, j);
        p.increment_index<factor>(data_i.get_ff_type(i), res.distance_bin, res.distance);
    }
    ...
    inline void evaluate16(
        ...
        xyzff::HexaEvaluatedResultRounded res = add16::evaluate_unweighted(data_i, data_j, i, j);
        for (int k = 0; k < 16; ++k) {p.increment_index<factor>(data_i.get_ff_type(i), res.distance_bins[k]);}
    }
    ...
    inline void evaluate8(
        ...
        xyzff::OctoEvaluatedResultRounded res = add8::evaluate_unweighted(data_i, data_j, i, j);
        for (int k = 0; k < 8; ++k) {p.increment_index<factor>(data_i.get_ff_type(i), res.distance_bins[k]);}
    }
    ...
    inline void evaluate4(
        ...
        xyzff::QuadEvaluatedResultRounded res = add4::evaluate_unweighted(data_i, data_j, i, j);
        for (int k = 0; k < 4; ++k) {p.increment_index<factor>(data_i.get_ff_type(i), res.distance_bins[k]);}
    }
