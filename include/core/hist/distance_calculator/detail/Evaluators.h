...
namespace ausaxs::hist::detail {
    ...
    /**
     * @brief Add a pair at @a distance to its @a bin, counted @a factor times.
     *        With @a unit_weights the pair weighs 1, and @a weight is never read, since the kernels leave it unset.
     */
    template<int factor, bool unit_weights, BinEntry Entry>
    inline void accumulate(std::span<Entry> p, std::int32_t bin, float distance, [[maybe_unused]] const float* weight) {
        ...
        if constexpr (unit_weights) {
            if constexpr (std::is_same_v<Entry, WeightedEntry>) {
                p[bin].template add<factor>(distance, 1.0);
            } else {
                p[bin] += factor;
            }
        } else if constexpr (std::is_same_v<Entry, WeightedEntry>) {
