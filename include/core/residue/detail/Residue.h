...
namespace ausaxs::residue::detail {
    ...
    struct ResidueAtom {
         ...
         */
        ResidueAtom(std::string name, std::string altname, constants::atom_t symbol);
         ...
         * @param atom The atom type. This is necessary to avoid ambiguities since the name is always capitalized in PDB files, so otherwise we cannot distinguish between e.g. a C-alpha (CA) and a calcium (Ca).
         */
        ResidueAtom(std::string name, int charge, constants::atom_t symbol);
         ...
         */
        void add_bond(constants::atom_t symbol, int order);
    ...
    struct Bond {
         ...
         * @param order the order of the bond.
         */
        Bond(std::string name1, std::string name2, int order);
         ...
         */
        static int parse_order(const std::string& order);
    ...
    class Residue {
        public: 
             ...
             */
            Residue(std::string name, std::vector<ResidueAtom> atoms, const std::vector<Bond>& bonds);
            ...
            std::vector<ResidueAtom>& get_atoms() {return atoms;}
            const std::vector<ResidueAtom>& get_atoms() const {return atoms;}
        ...
        private: 
            ...
            std::vector<ResidueAtom> atoms;        
    };

    std::ostream& operator<<(std::ostream& os, const residue::detail::ResidueAtom& a);
