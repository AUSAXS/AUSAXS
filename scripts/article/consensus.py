...
for file in os.listdir(folder):
    ...
    if os.path.isdir(os.path.join(folder, file)):
        ...
        for nested_file in os.listdir(os.path.join(folder, file)):
            ...
            name = "".join(tokens[1:]).split(".")[0]
            if name.endswith("stripped"): name = name[:-8]
            if name.endswith("WAXSiS"): name = name[:-6] + "_MD"
...
def plot_one(data, x_labels, method):
    ...
    plt.tight_layout()
    plt.savefig(f"output/consensus/{method}_comparison.png", dpi=300)
    ...
    plt.tight_layout()
    plt.savefig(f"output/consensus/{method}_comparison_flipped.png", dpi=300)
...
plt.yticks(np.arange(len(y_labels)), y_labels)
plt.tight_layout()
plt.savefig(f"output/consensus/difference.png", dpi=300)
...
plt.tight_layout()
plt.savefig(f"output/consensus/difference_flipped.png", dpi=300)
