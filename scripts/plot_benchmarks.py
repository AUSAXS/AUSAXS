...
if data["ausaxs_simple"]:   plt.errorbar(ausaxs_simple[:, 0],       ausaxs_simple[:, 1],        yerr=ausaxs_simple[:, 2],    fmt='.', color='purple',    label="$AUSAXS_{simple}$")
if data["ausaxs_simple_st"]:plt.errorbar(ausaxs_simple_st[:, 0],    ausaxs_simple_st[:, 1],     yerr=ausaxs_simple_st[:, 2], fmt='.', color='pink',      label="$AUSAXS_{simple,\ st}$")
if data["ausaxs_fraser"]:   plt.errorbar(ausaxs_fraser[:, 0],       ausaxs_fraser[:, 1],        yerr=ausaxs_fraser[:, 2],    fmt='.', color='green',     label="$AUSAXS_{fraser}$")
if data["ausaxs_grid"]:     plt.errorbar(ausaxs_grid[:, 0],         ausaxs_grid[:, 1],          yerr=ausaxs_grid[:, 2],      fmt='.', color='red',       label="$AUSAXS_{grid}$")
if data["ausaxs_serial"]:   plt.errorbar(atom_mean,                 ausaxs_ser[:,1],            yerr=ausaxs_ser[:, 2],       fmt='.', color='brown',     label="$AUSAXS_{serial}$")
